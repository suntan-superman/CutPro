...
try {
  ...
  observe(page);
  const traffic = observeDirectUploadTraffic(page, { origin, projectUrl, base: "/api/estimate" });
  const photoA = !resumeId ? await makePhonePhoto({ name: "CUTPRO-QA-estimate-A.jpg", seed: 13, marker }) : null;
  const photoB = !resumeId ? await makePhonePhoto({ name: "CUTPRO-QA-estimate-B.jpg", seed: 17, marker }) : null;
  const fields = {
  ...
  if (!resumeId) {
  check("two source phone JPEGs genuinely exceed 4.5 MiB and stay below 8 MiB each", [photoA, photoB].every((photo) => photo.buffer.length > 4.5 * 1024 * 1024 && photo.buffer.length <= 8 * 1024 * 1024));
  check("private source photos contain realistic metadata", Boolean((await sharp(photoA.buffer).metadata()).exif));
  ...
  const metrics = await traffic.certify({ minimumLargePhotos: 2 });
  check("large estimate photos travel directly to private Storage while app requests contain only small JSON", metrics.directUploads === 2 && metrics.largeDirectUploads === 2);
  }
  ...
  check("lead job/services/urgency/notes persisted", lead.job_description === "CUTPRO ADMIN CERTIFICATION TEST — NOT A CUSTOMER LEAD" && lead.urgency === "Within a week" && lead.services.includes("tree-trimming") && lead.services.includes("stump-grinding") && lead.approximate_count === "2 trees and 1 stump — TEST ONLY" && lead.preferred_timeframe === "QA only — no appointment requested" && lead.customer_notes === marker && lead.source === "estimate" && (resumeId || lead.status === "new"));
  check("two photo references persisted", lead.photo_references.length === 2);
  if (!resumeId) {
    // Finalization is JSON-only. Replay the exact capability/reference captured
    // in memory, including two concurrent retries; never write it to artifacts.
    assert.ok(traffic.finalBody);
    const replays = await Promise.all(Array.from({ length: 2 }, () => fetch(`${origin}/api/estimate`, { method: "POST", headers: { "content-type": "application/json" }, body: traffic.finalBody })));
    for (const replay of replays) {
      const replayResult = await replay.json();
      check("concurrent replay returns the same reference as duplicate", replay.status === 200 && replayResult.duplicate === true && replayResult.reference === result.reference);
    }
  } else {
    console.log("RESUME: direct-upload transport/idempotency checks skipped because no original in-memory capability is retained.");
  }
  ...
  for (const photo of lead.photo_references) {
    ...
    if (!resumeId) {
      const original = [photoA, photoB].find((item) => item.name === photo.originalName);
      check("private phone photo retains its verified original bytes and full dimensions", Boolean(original) && metadata.format === "jpeg" && metadata.width === 4000 && metadata.height === 3000 && photo.storedType === "image/jpeg" && createHash("sha256").update(buffer).digest("hex") === createHash("sha256").update(original.buffer).digest("hex"));
      check("private phone photo size and original name are persisted", photo.originalSize === original.buffer.length && photo.storedSize === original.buffer.length);
    } else {
      check("retained QA photo still decodes as its recorded image type", ["jpeg", "png", "webp"].includes(metadata.format) && metadata.width > 0 && metadata.height > 0);
    }
  }
  ...
  await adminPage.locator("#statusFilter").selectOption(lead.status);
  ...
  await adminPage.locator("#statusFilter").selectOption(lead.status === "won" ? "new" : "won");
  await adminPage.getByRole("button", { name: "Apply filters", exact: true }).click();
